...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const DEFAULT_CONFIG = {
			 ...
			 *  ⚠ v0.15.0:悬浮 Pill 功能已注释下线(保留此块以便恢复),不再生效 */
			/* pill: {
				enabled: true,
		...
		/** 配置片段合并到默认配置(浅合并,gradient / title / danmaku 对象深合并;pill 已随功能下线) */
		...
		function apply(ctx) {
			...
			const updateStyle = () => {
				...
				const colors = (g && g.enabled && Array.isArray(g.colors) ? g.colors : []).filter(isSafeColorToken);
			...
			// 监听者通知合并到微任务:一次 engineTick 会连着 setLive 好几次
			// (phase / elapsed / running / tps / pending),逐次通知会让「已接管」
			// 元素每秒重渲染两三遍。合并后每拍最多一次。
			let liveFlushQueued = false;
			...
			const flushLiveListeners = () => {
				liveFlushQueued = false;
				if (liveListeners.size === 0) return;
				const listeners = Array.from(liveListeners);
			...
			const setLive = (patch) => {
				...
				if (!changed || liveFlushQueued) return;
				liveFlushQueued = true;
				// 优先用微任务(同一拍合并);环境没有 queueMicrotask 时退回同步通知
				if (typeof queueMicrotask === "function") queueMicrotask(flushLiveListeners);
				else flushLiveListeners();
			};
			...
			const wireLiveEngine = () => {
				...
				// 文案/标题一旦含动态占位符,就跟着实时状态刷新:liveTickMs 定时器只负责
				// 「算」({elapsed}/{tps} 等),这里负责把结果立刻画到屏幕上 ——
				// {pending} 这类事件驱动的字段因此不必等下一次轮换。
				// (订阅放在这里而不是声明处:renderLiveNow 会调用 updateTitle,
				//   放到声明处会踩到 updateTitle 的 TDZ。)
				subscribeLive(renderLiveNow);
			...
			const rescanAll = () => {
				...
				for (const el of adopted) if (el.isConnected) syncTruncation(el);
			};
			...
			const applyConfig = () => {
				...
				// v0.15.0:原「Pill 开启即运行」的触发条件已随 Pill 功能下线
			...
			const SETTINGS_DICTS = {
				...
				en: {
					...
					/* v0.15.0: pill strings removed with the feature
			...
			const SETTINGS_CSS =
				...
				/* 页脚:设置页最底部的仓库链接(上一条分组自带分隔线,这里不再重复画线) */
				...
				/* v0.15.0:实时状态 Pill 浮层样式已移除(原 .dsh-sr-pill* 四条规则) */
				"";
			...
			const SettingsPanel = (props) => {
				...
				/* v0.15.0:pillDraft 已随 Pill 功能注释下线
				...
				*/
				...
				/** 词库包编辑目标:"" = 默认词库,否则包 id(可独立编辑包内文案) */
				const [packSel, setPackSel] = react.useState("");
				const packSelRef = react.useRef("");
				const setPackSelBoth = (id) => {
					packSelRef.current = id;
					setPackSel(id);
				};
				/** 已启用的词库包 id 列表(空数组 = 全部关闭;) */
				const [packEnabled, setPackEnabled] = react.useState([]);

				const applyDoc = react.useCallback((data, presetId, packId) => {
				...
				const load = react.useCallback(async () => {
					...
					try {
						...
						if (packSelRef.current && !packs.some((p) => p.id === packSelRef.current)) packSelRef.current = "";
						// 默认(未写 enabledPacks)= 全部启用,UI 显示全开;若默认词库为空则自动选中第一个包
						...
						if (!packSelRef.current && coreEmpty && packs.length > 0) packSelRef.current = packs[0].id;
						setPackSel(packSelRef.current);
						...
						applyDoc(data, selRef.current, packSelRef.current);
				...
				const save = react.useCallback(async () => {
					...
					// 词库包编辑目标:选中包时把词库写进包;启用开关随文档保存
					const packTarget = packOf(next, packSelRef.current);
					writeGroups(packTarget || holder);
					const badColorTokens = invalidColorList(gradientDraft.colors).concat(invalidColorList(danmakuDraft.colors));
					if (badColorTokens.length > 0) {
						setStatus({ kind: "error", text: t("gradient.invalidColor") + " " + badColorTokens.join(", ") });
						return;
					}
				...
				}, [basic, doc, drafts, scheduleDrafts, gradientDraft, danmakuDraft, weighted, packEnabled, t]);
				...
				const previewColors = parseColorList(gradientDraft.colors);
				...
				const previewText = (String(d[PHASE_THINKING] || "").split(/\r?\n/).find((line) => line.trim().length > 0) || t("preview.sample")).trim();
				const previewStyle = {
				...
				return react.createElement("div", { className: "dsh-sr-settings" },
					...
					// 文案词库:目标选择 + 语言切换 + 每阶段一个 textarea
					react.createElement("section", { className: "dsh-sr-group" },
						react.createElement("div", { className: "dsh-sr-grouphead" },
							react.createElement("h3", null, t("library")),
							react.createElement("span", { className: "dsh-sr-muted" },
								t("packs.editing").replace("{name}", packSel ? (labelOf(packOf(doc, packSel), editLang) || packSel) : t("packs.target.base")))
						),
						react.createElement("label", { className: "dsh-sr-field" },
							...
							react.createElement("select", {
								...
								disabled: locked,
								onChange: (event) => {
									setPackSelBoth(event.target.value);
									applyDoc(doc, selRef.current, event.target.value);
								},
							},
								react.createElement("option", { value: "" }, t("packs.target.base")),
								packs.map((p) => react.createElement("option", { key: p.id, value: p.id }, labelOf(p, editLang) || p.id))
							)
						),
					...
					react.createElement("section", { className: "dsh-sr-group" },
						...
						react.createElement("div", { className: "dsh-sr-btnrow" },
							presetOptions(sel, (event) => {
								...
								applyDoc(doc, event.target.value, packSelRef.current);
							}),
			...
			/* ── v0.15.0:实时状态 Pill 功能已注释下线(保留完整实现,恢复时取消本注释块
			    并还原内部两处 JSDoc 的 "* /" 写法) ─────────────────────────────
