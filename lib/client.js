	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			const engineTick = () => {
				...
				lastCharsTime = now;
				// DOM 对 phase/running/elapsed 保持权威(与状态文案同一套判定):
				// 回合结束元素消失 → 立即回到 idle,不依赖会话 API。
			...
			const SettingsPanel = (props) => {
				...
				}, [basic, doc, drafts, scheduleDrafts, pillDraft, t]);
