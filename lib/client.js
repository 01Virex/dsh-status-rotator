 ...
 * for a random phrase from PHRASES, rotating every INTERVAL_MS. The
 * elapsed-time clock (appears after 15s) is left untouched.
 *
 * Customization: edit PHRASES / INTERVAL_MS below, then Ctrl+F5. Runtime
 * overrides via localStorage are also supported (see README): priority
 * texts.<locale> > texts > built-in PHRASES. Phrases follow the DSH UI
 * language (zh / en) live; unknown locales fall back to zh.
 */
