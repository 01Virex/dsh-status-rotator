		...
		function apply(ctx) {
			...
			const adopt = (el) => {
				...
				if (el.getAttribute("role") === "status") {
					...
					applyText(el);
					if (badge !== null) badge.textContent = `rotator: label=${document.querySelectorAll('[role="status"]').length} adopted=${adopted.size}`;
				}
			};
			...
			const rescanAll = () => {
				...
				if (badge !== null) badge.textContent = `rotator: label=${status.length} adopted=${adopted.size}`;
			};
			...
			const start = () => {
				...
				// 屏幕角标:有它在,说明这个页面运行着插件
				badge = document.createElement("div");
				badge.setAttribute("aria-hidden", "true");
				badge.textContent = "rotator…";
