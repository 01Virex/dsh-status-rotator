...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 配置片段合并到默认配置(浅合并,gradient / title 对象深合并) */
		function mergeConfig(base, over) {
			if (!over) return { ...base };
			const out = { ...base };
			for (const key of Object.keys(over)) {
				if ((key === "gradient" || key === "title") && over[key] !== null && typeof over[key] === "object") {
		...
		const isDynamicTemplate = (template) => /\{(elapsed|date|time)\}/.test(String(template || ""));
		...
		function apply(ctx) {
			...
			/** 模板变量的上下文(el 为 null = 无回合/idle 场景) */
			const ctxFor = (el) => {
				const now = new Date();
				const pad = (n) => String(n).padStart(2, "0");
				const base = {
				...
				if (el === null) {
					return { ...base, elapsed: formatElapsed(0, lastLocale), phase: "idle", phaseLabel: labels.idle };
				}
			...
			const SettingsPanel = (props) => {
				...
				const save = react.useCallback(async () => {
					...
					if (target) {
						target.config = { ...(target.config && typeof target.config === "object" ? target.config : {}), ...numbers };
						writeGroups(target);
					} else {
				...
				}, [basic, doc, drafts, scheduleDrafts, t]);
