...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			/** 0.1.7 宿主:可见标签的 class 名带构建哈希,只能靠 data-turn-process 按钮定位 */
			...
			/** 0.1.7 宿主:被接管的按钮(React 的标签元素藏在里面,插件 span 也挂在它上面) */
			const TURN_ROOT_CLASS = "dsh-status-rotator-turn";
			 ...
			 * 0.1.7 的宿主是按钮(display:flex / width:100%),插件文案 span 与时钟 span
			 * 挂在它上面,字号 / 行高对齐 dsh 标签自己的规则(同一组 CSS 变量,跟随
			 * 「内容字号」设置),被藏起来的 React 标签不参与布局。
			 */
			...
			layoutStyleEl.textContent =
				...
				"." + TURN_ROOT_CLASS + " > ." + TEXT_SPAN_CLASS + "{" +
				"flex:0 1 auto;font-size:var(--dsh-content-font-size-secondary,13px);" +
				"line-height:calc(24px + var(--dsh-content-font-delta,0px))}" +
				"." + CLOCK_CLASS + "{flex:none;margin-left:8px;font-size:.92em;font-variant-numeric:tabular-nums}" +
			...
			// ══ dsh 0.1.7+ 状态行:接管 button[data-turn-process] ══
			//
			// 0.1.7 把回合状态拆成两半:一个视觉隐藏的 role="status" aria-live 公告
			// span(内容仍是 "Deep diving..."/"深度求索中",专供读屏),以及按钮里真正
			// 可见的标签 span("Deep diving for 12s" —— 时长并进同一段文本,每秒重写)。
			// 旧逻辑只认 role="status",于是接管的是那个隐藏公告:文案确实被换掉了,
			// 但界面上看不见,而可见的 "Deep diving for 12s" 原样留着 —— 这就是
			// 「dsh 0.1.7 里替换 deep diving 不生效」的真正原因。
			//
			// 接管方式(实测线上 0.1.7 GUI 定的形):React 提交阶段对「单个字符串子
			// 节点」走的是 setTextContent —— 也就是每秒把标签元素的 textContent 整段
			// 重写,而不是改文本节点的 nodeValue。往标签里塞插件自己的 span 会被下一次
			// 重写整个抹掉(holder 与时钟一起消失、阶段判定只能一直 thinking)。所以这里
			// 接管的是**按钮本身**:
			//   · React 的标签元素 display:none 藏起来(它继续每秒被重写,但看不见);
			//   · 插件自己的文案 span 与时钟 span 挂在按钮上(React 不认识它们,不会动);
			//   · 时长从藏起来的标签文本里原样读出来(12s / 1分02秒 / 2m 04s),
			//     phase / {elapsed} / 打字机锁宽都按老路径继续工作;
			//   · 标签文本不再是运行中文案(Worked / Took 12s / Stopped / Failed…)时
			//     说明回合结束,立刻撤掉插件 span 并恢复标签显示。
			...
			/** 0.1.7 宿主:按钮 → React 那个会被每秒重写的标签元素 */
			const turnLabels = new Map();
			/** 0.1.7 宿主:按钮 → 上次判定出的阶段(阶段变化时立刻换文案,不等轮换) */
			const turnPhases = new Map();

			/**
			 * 宿主每秒重写标签文本。React 走 setTextContent → 标签内部是 childList
			 * 变化,而主观察器只盯 document 的 childList/attributes —— 所以对每个
			 * data-turn-process 按钮单独挂一个观察器:
			 *   · 已接管 → 同步时长 / 判断回合是否结束;
			 *   · 未接管 → 重新判定(回合结束后宿主写 Worked / Took 12s,下一次回合
			 *     开始又写回运行中文案,这样能立刻接管,不用等 2 秒兜底轮询)。
			 * 插件自己的 span(文案/时钟)里的写入要跳过,否则打字机会把自己叫醒。
			 */
			...
			const sourceObserver = new MutationObserver((records) => {
				...
				for (const record of records) {
					...
					if (node.closest("." + TEXT_SPAN_CLASS + ",." + CLOCK_CLASS)) continue;
					const button = node.closest(TURN_PROCESS_SELECTOR);
					if (!button) continue;
					if (adopted.has(button)) {
						if (!synced.has(button)) {
							synced.add(button);
							syncTurnProcessHost(button);
						}
						continue;
					}
			...
			/** 插件自己的 span(按钮上除 React 标签与 chevron 之外的那两个) */
			const isOwnSpan = (el) => el.classList && (el.classList.contains(TEXT_SPAN_CLASS) || el.classList.contains(CLOCK_CLASS));

			/**
			 * 0.1.7 宿主:取按钮里 React 的标签元素。优先用接管时记下的那个
			 * (它被 display:none 藏着,不能按"可见"过滤);宿主重渲染换了元素时,
			 * 按「不是插件自己的 span、且有文本」重新认领。
			 */
			...
			/** 藏起 React 的标签(幂等);它在原地继续被每秒重写,只是看不见 */
			const hideTurnLabel = (labelEl) => {
				try {
					if (labelEl.style.display !== "none") labelEl.style.display = "none";
				} catch (error) { /* ignore */ }
			};

			/** 结构改造(只做一次):藏起 React 标签,把插件文案 span 与时钟 span 挂到按钮上 */
			const buildTurnProcessHost = (button, labelEl) => {
				turnLabels.set(button, labelEl);
				hideTurnLabel(labelEl);
				if (!button.querySelector(":scope > ." + TEXT_SPAN_CLASS)) {
					const text = document.createElement("span");
					text.className = TEXT_SPAN_CLASS;
					text.appendChild(document.createTextNode(""));
					button.appendChild(text);
				}
				if (!button.querySelector(":scope > ." + CLOCK_CLASS)) {
					const clock = document.createElement("span");
					clock.className = CLOCK_CLASS;
					clock.setAttribute("aria-hidden", "true");
					button.appendChild(clock);
				}
				button.classList.add(HOST_CLASS, TURN_ROOT_CLASS);
			};

			/** 释放 0.1.7 宿主:撤掉插件 span/class、恢复标签显示,重新交给 React */
			const releaseTurnProcessHost = (button) => {
				try {
					const text = button.querySelector(":scope > ." + TEXT_SPAN_CLASS);
					if (text) text.remove();
					const clock = button.querySelector(":scope > ." + CLOCK_CLASS);
					if (clock) clock.remove();
					const labelEl = turnLabels.get(button);
					if (labelEl) labelEl.style.removeProperty("display");
					button.classList.remove(HOST_CLASS, HOST_GRADIENT_CLASS, TURN_ROOT_CLASS);
					button.style.removeProperty("min-width");
				} catch (error) { /* ignore */ }
				const state = typists.get(button);
				if (state && state.timer !== null) clearInterval(state.timer);
				typists.delete(button);
				clearLive(button);
				lastPicks.delete(button);
				liveTemplates.delete(button);
				turnPhases.delete(button);
				turnLabels.delete(button);
				adopted.delete(button);
				log("released turn-process host");
			};

			/**
			 * 把藏起来的标签文本里的时长同步到插件时钟 span,并按阶段变化换文案。
			 * 文本不再是「运行中」文案(Worked / Took 12s / Failed / Stopped…)时,
			 * 说明回合已经结束 —— 立刻释放,让宿主的结束文案重新可见。
			 */
			const syncTurnProcessHost = (button) => {
				const labelEl = turnLabelOf(button);
				if (!labelEl) { releaseTurnProcessHost(button); return; }
				...
				if (duration === null) { releaseTurnProcessHost(button); return; }
				hideTurnLabel(labelEl);
				const clock = clockEl(button);
				if (clock && clock.textContent !== duration) clock.textContent = duration;
				const phase = phaseOf(button);
				const previous = turnPhases.get(button);
				turnPhases.set(button, phase);
				// 时钟出现(thinking → running)或跨过 longAfterMs 时立刻换文案,
				// 与旧宿主「时钟出现即切换」的手感一致;首次接管由 adopt 自己 refresh。
				if (previous !== undefined && previous !== phase) refresh(button);
			};

			/** 接管 0.1.7 宿主按钮:结构改造 → 同步时钟 → 选第一句文案 */
			const adoptTurnProcessHost = (button, labelEl) => {
				if (adopted.has(button)) {
					// 宿主重渲染换了标签元素:重新认领并藏好
					turnLabels.set(button, labelEl);
					hideTurnLabel(labelEl);
				...
				if (adopted.has(labelEl)) adopted.delete(labelEl);   // 旧版本误接管过标签元素
				buildTurnProcessHost(button, labelEl);
				adopted.add(button);
				turnPhases.set(button, undefined);
				syncTurnProcessHost(button);
				log("adopted turn-process button, 宿主文本:", JSON.stringify((labelEl.textContent || "").slice(0, 40)));
				if (adopted.has(button)) {
					refresh(button);
					updateTitle();
				}
			...
			const scanTurnProcess = (root) => {
				...
				for (const button of buttons) {
					...
					for (const child of Array.from(button.children)) {
						...
						adoptTurnProcessHost(button, child);
						break;
					}
				}
			};
			...
			const rescanAll = () => {
				...
				for (const el of adopted) {
					if (!el.isConnected) continue;
					if (el.classList.contains(TURN_ROOT_CLASS)) syncTurnProcessHost(el);
					syncTruncation(el);
				}
			};
			...
			const editorSignature = (state) => JSON.stringify([
				state.basic, state.weighted, state.gradientDraft, state.danmakuDraft,
			...
			const SettingsPanel = (props) => {
				...
				/** 草稿与基线不一致 = 还有没落盘的改动 */
				const dirty = baseline !== "" && editorSignature({ basic, weighted, gradientDraft, danmakuDraft, drafts, scheduleDrafts, packEnabled }) !== baseline;
				...
				const applyDoc = react.useCallback((data, target) => {
					...
					return { basic: nextBasic, weighted: nextWeighted, gradientDraft: nextGradient, danmakuDraft: nextDanmaku, drafts: nextDrafts };
				}, []);
			...
			ctx.effect(() => {
				return () => {
					...
					for (const el of adopted) {
						try {
							// 0.1.7 宿主:撤掉插件 span/class、恢复 React 标签显示
							if (el.classList.contains(TURN_ROOT_CLASS)) {
								releaseTurnProcessHost(el);
								continue;
							}
							el.style.removeProperty("min-width");
