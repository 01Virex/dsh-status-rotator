		...
		function apply(ctx) {
			...
			const SETTINGS_DICTS = {
				zh: {
					...
					"packs.hint": "启用的包会与默认词库按文本去重合并;选中某包后,下方「文案词库」编辑区读写该包。未设置 enabledPacks 时全部包自动启用。",
					"pack.items": "{n} 条",
					"schedule": "时段调度",
					"schedule.add": "添加规则",
					"schedule.remove": "删除",
				...
				en: {
					...
					"packs.hint": "Enabled packs merge into the default bank (deduped by text). Pick a pack to read/write its phrases in the library editor below. When enabledPacks is unset, all packs are enabled.",
					"pack.items": "{n} items",
			...
			const SettingsPanel = (props) => {
				...
				const load = react.useCallback(async () => {
					...
					try {
						...
						setPackEnabled(Array.isArray(data && data.enabledPacks) ? data.enabledPacks.slice() : []);
				...
				}, [basic, doc, drafts, scheduleDrafts, pillDraft, gradientDraft, danmakuDraft, weighted, packEnabled, t]);
