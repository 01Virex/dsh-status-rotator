...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 解析时钟文本("0:15" / "1:02:03" / "42" / "42s")为秒数,解析失败返回 0 */
		...
		function apply(ctx) {
			...
			function phaseOf(el) {
				const clock = Array.from(el.childNodes).find((n) => n.nodeType === 1);
			...
			const adopt = (el) => {
				...
				// 只接管 TurnStatus:role="status" + aria-live="polite" 的组合在页面
				// 上唯一。不能只按 role 匹配(输入栏 notice、重试行等 aria-live 区域
				// 也用它),也不能按文本匹配(聊天记录里的代码片段会误伤)。
