	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			const resolveDanmakuMount = () => {
				...
				if (z >= 0) return { parent: document.body, mode: "fixed", z: String(z), frame: null };
				const frame = appFrameOf();
				const plan = danmakuMountPlan(z, frame !== null);
				return { parent: frame || document.body, mode: plan.mode, z: plan.z, frame };
			};

			/** 兜底挂到 body 之后短时间内再试一次主框架(外壳可能刚好渲染完) */
			 ...
			 * zIndex >= 0 时挂在 body 上浮于界面之上,层级 = zIndex。
			 *
			 * 与旧实现的差别:不再「层存在就早退」,而是比对当前挂载目标,目标变了
			 * (例如插件早于外壳渲染时先落到 body 兜底)就重建 —— 这正是弹幕失效的根因。
			 */
			const ensureDanmakuLayer = () => {
				...
				if (mount.mode === "frame") {
					try {
						danmakuFrameSavedIsolation = mount.frame.style.isolation;
						mount.frame.style.isolation = "isolate";
					} catch (error) { /* ignore */ }
				}
				...
				if (mount.mode === "frame") {
					log("danmaku layer mounted inside the app frame");
				} else if (mount.z === "1") {
			...
			/** 摘掉弹幕层与在途弹幕(不动发射定时器);恢复主框架 isolation */
