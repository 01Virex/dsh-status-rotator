 ...
 *         bundled config.example.json → package config.json → official
 *         settings namespace (`$DSH_HOME/settings.yaml`, survives plugin
 *         upgrades) → external phrase bank
 *         (`$DSH_HOME/status-rotator/phrases.json`, or
 *         `$DSH_STATUS_ROTATOR_BANK`; re-read whenever the file changes).
 *         The bank only carries `packs` / `phrases`.
 *   PUT/POST  same route
...
/**
 * 外部词库的加载缓存。为什么要缓存而不是每次读:GET 路由每 `reloadIntervalMs`
...
/**
 * 读取外部词库覆盖层:进程运行期间检测文件变更并重载,不需要重启、不需要重装包。
 *
 * 语义:
 *   - 文件不存在 → 返回 null,生效文档完全按内置默认词库(内置词库是兜底);
 *   - 文件是合法 JSON 文档(与 config.example.json 同构,**只写要覆盖的键**即可)
 *     → 作为最高优先级的词库层参与 mergeLayers(内置词库 → config.json → 设置
 *     存储 → 外部词库);文案数组成员按现有语义整体替换,带唯一 id 的词库包按 id
 *     逐条合并,所以只写一个包的一个阶段即可,其余包原样保留;
 *     只认 `packs` / `phrases` 两个键,其它键(例如 `config`)忽略;
 *   - 文件损坏或结构校验不过 → 记下 error、保留上一次成功加载的值,不把线上词库打挂;
 *     文件被删除后,下一次请求回落到内置词库。
 */
async function externalBankDocument() {
	...
	try {
		...
		if (externalBankCache.path !== path || externalBankCache.doc !== null || externalBankCache.text !== null) {
			externalBankCache = {
				path,
				mtimeNs: null,
				size: -1,
				text: null,
				doc: null,
				reloads: externalBankCache.path === path ? externalBankCache.reloads : 0,
	...
	if (externalBankCache.path === path && externalBankCache.mtimeNs === mtimeNs && externalBankCache.size === size) {
		return externalBankCache.doc;
	}
	let text;
	try {
		...
		return externalBankCache.path === path ? externalBankCache.doc : null;
	}
	if (externalBankCache.path === path && externalBankCache.text === text) {
		externalBankCache = { ...externalBankCache, mtimeNs, size };
		return externalBankCache.doc;
	}
	const kept = externalBankCache.path === path ? externalBankCache : null;
	let doc = null;
	let error = null;
	try {
		const parsed = validateConfigDocument(JSON.parse(text));
		// 词库文件只贡献词库:只取 `packs` / `phrases` 两个键,其余键(例如 config)
		// 一律忽略 —— 运行时配置继续由设置页 / config.json 管理,外部文件不越权。
		doc = {};
		if (parsed.phrases !== undefined) doc.phrases = parsed.phrases;
		if (parsed.packs !== undefined) doc.packs = parsed.packs;
	} catch (parseError) {
		error = String(parseError.message || parseError);
	}
	if (error !== null) {
		externalBankCache = { path, mtimeNs, size, text, doc: kept ? kept.doc : null, reloads: kept ? kept.reloads : 0, error };
		return externalBankCache.doc;
	}
	externalBankCache = { path, mtimeNs, size, text, doc, reloads: (kept ? kept.reloads : 0) + 1, error: null };
	return doc;
}

/** 外部词库的加载状态(诊断 / 测试用;不参与请求处理) */
 ...
 * 完整词库(12 个包 1077 条)→ 每次装载/保存都把 70+ KB 词库序列化进
...
/** 生效配置文档:内置默认文档 → 插件目录文件层 → settings 用户层 */
async function effectiveConfigDocument(getSettingsApi) {
	...
	// 外部词库是**可选**的词库层,压在最后:内置词库兜底,插件目录 config.json 与设置
	// 存储的 config 语义都不变;词库文件声明了的包就以它为准(改完即生效,见
	// externalBankDocument)。它只带 packs / phrases,不会碰运行时配置。
	const bankDoc = await externalBankDocument();
	return mergeLayers(await bundledDocument(), fileDoc, userDoc, bankDoc);
}
...
function contentTypeOf(doc) {
	...
	const out = { ...doc };
	delete out[SETTINGS_VERSION_KEY];
	return out;
}
...
async function saveConfig(req, res, getSettingsApi) {
	...
	// 只写「与随包默认文档的差异」:整份词库留在包内的 config.example.json,
	// 设置命名空间不再跟着词库一起膨胀。
	const s = await getSettingsApi();
	if (s) {
		try {
			const bundled = await bundledDocument();
			const stored = bundled ? { ...deltaOf(bundled, document), [SETTINGS_VERSION_KEY]: SETTINGS_VERSION } : document;
