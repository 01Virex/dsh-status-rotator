...
/**
 * 只保留 userDoc 相对 baseDoc 真正不同的部分(深层递归;数组整体替换)。
 *
 * 为什么需要:设置命名空间原本按顶层键整份覆盖,而提交上来的文档永远带着
 * 完整词库(12 个包 1074 条)→ 每次装载/保存都把 70+ KB 词库序列化进
 * `$DSH_HOME/settings.yaml`,配置文件和词库一起膨胀。
 *
 * 语义与 dsh-settings 的分层一致(对象递归合并、数组整体替换),所以
 * `mergeDocuments(bundled, deltaOf(bundled, doc))` 与 doc 等价;
 * 两边相同的键不进 delta,删除的键自然表现为「base 里有、delta 里没有」。
 * 纯函数,供测试。
 */
...
async function effectiveConfigDocument(getSettingsApi) {
	...
	return mergeLayers(await bundledDocument(), fileDoc, userDoc);
}
...
async function trimSettingsSection(s) {
	...
	if (fileDoc && bundled) {
		...
		next = { ...deltaOf(bundled, fileDoc), [SETTINGS_VERSION_KEY]: SETTINGS_VERSION };
	} else if (hasStored && bundled) {
		next = { ...deltaOf(bundled, stored), [SETTINGS_VERSION_KEY]: SETTINGS_VERSION };
	} else {
