...
/**
 * 接入官方持久设置(ctx.settings → $DSH_HOME/settings.yaml,升级插件不清空)。
 * 服务/模块不可用时静默返回 null,插件继续走文件式 config.json(旧行为)。
 */
async function initializeSettings(ctx) {
	let settings = null;
	try {
	...
	try {
		...
		// 宽松 schema:接受任意 JSON 对象(结构校验由本插件的 validateConfigDocument 把关)
		const schema = z.object({}).loose();
		const scope = settings.register(ns, schema, { applies: "live" });
		return { settings, ns, scope };
	} catch (error) {
		return null;
	}
}
...
async function effectiveConfigDocument(settingsPromise) {
...
/** 一次性迁移:settings 尚无用户配置且插件目录存在 config.json 时,导入一次(升级保值) */
async function migrateConfigOnce(settingsPromise) {
	const s = await settingsPromise;
	if (!s) return;
	try {
		const v = s.scope.get();
	...
	if (!doc) return;
	try {
		await s.scope.replace(doc);
	} catch (error) { /* ignore */ }
}

function createServeConfig(settingsPromise) {
	return async function serveConfig(req, res) {
		if (req.method === "GET" || req.method === "HEAD") {
			const doc = await effectiveConfigDocument(settingsPromise);
			if (doc === null) {
				res.writeHead(404);
				res.end();
				return;
		...
		if (req.method === "PUT" || req.method === "POST") {
			await saveConfig(req, res, settingsPromise);
			return;
		}
		res.writeHead(405);
		res.end();
...
async function saveConfig(req, res, settingsPromise) {
	let raw;
	try {
...
function apply(ctx) {
	// 官方持久设置(schemastery/dsh-settings 缺失或未挂载时静默降级为文件式)
	const settingsPromise = initializeSettings(ctx);
	// 一次性迁移:把已保存的 config.json 导入 settings(升级保值的关键一步)
	migrateConfigOnce(settingsPromise);
	// webServer 可选:缺失时插件照常激活(客户端注入由 dsh 自己管理),只是没有配置路由
	let webServer = null;
	try {
	...
	if (webServer && typeof webServer.register === "function") {
		...
		ctx.effect(() => webServer.register({
			...
			handler: createServeConfig(settingsPromise)
