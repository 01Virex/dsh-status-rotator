...
- **一次设置页保存会把外部词库的内容当成「用户改动」冻进存储,README 承诺的「把包从外部词库删掉即可回到设置页管理」因此失效**([#92](https://github.com/01Virex/dsh-status-rotator/issues/92),报告人 [@mulinyang041](https://github.com/mulinyang041);报告文本由报告人机器上的 DSH agent 写就、账号主人授权发布)。
  - **根因**(报告人定位准确,我复核 + 端到端补齐):GET 返回的是**所有层合并后的整份文档**(外部词库优先级最高),设置页又把它整份 PUT 回来;而保存时算「用户改动」的基准只有「内置 + 自动更新词库」,**不含外部词库层** —— 词库塞进去的包于是被记成用户改动。存储层优先级高于 file / remote 层,所以之后按 README 把该包从外部词库删掉时,存储里那份快照会顶上来。
  - **还有一半在镜像上**(报告人标为「没往下追」的那点):保存时写进包目录的兼容镜像 `config.json` 是「整份已生效文档」,里面同样带着词库内容 —— 只修差异基准还不够,镜像会把那份内容继续供出来(上游内容同理:它**没**被冻进存储,但被镜像留住了)。
  - **修法**:① 判定基准改为 [`userChangeBaselineDocument()`](./lib/index.js)—— 内置 + 自动更新词库 + **外部词库**(三者都不是用户改的设置);`absorbFileLayer` 与老宿主设置存储走同一基准;② 兼容镜像只写**用户自己的配置**(内置 + 老宿主设置 + 本次算出的用户差异),刻意不含外部词库与自动更新这两层 —— 它们各有交付通道(词库文件 / 上游缓存)。
  - **防回归**:新增 [`scripts/verify-external-bank-ownership.cjs`](./scripts/verify-external-bank-ownership.cjs) —— 真实插件 + 真实 HTTP 路由 + 真实词库文件 + 整份 PUT 跑完整时间线,13 项断言(词库 / 上游 / 用户真改动三组对照);**修前 5 项红(A③A④A⑥、B③B④),修后全绿**,已进 CI 的 test 档。冒烟另加 2 条纯函数断言(外部词库内容不进设置;用户自己改的词条仍进设置)。
...
- **设置页「共 0 个包,已启用 10 个」「暂无词库包」,内置词库整个消失(外部反馈)。**
  保存路径把「提交上来的文档里没有某个包」当成用户删包写进存储(墓碑),而设置页的词库包区
  **只有启用 / 停用开关,没有删除按钮**(预设才有删除)—— 所以那只可能是文档不完整
  (设置页开着时插件升级 / 上游新增了包 / 这次提交的文档是残缺的),不是用户删的。
  一旦记成删除,生效文档就成了 `packs: []` 而 `enabledPacks` 还剩 10 条:设置页显示 0 个包、
  内置词库全部失效(只剩用户自己写过的那些词条),而且此后每次保存都会把这份残缺文档再存一遍,
  永久卡住。
  - **修法**:保存路径(用户配置存储 + 老宿主设置存储)不再写词库包的删除墓碑 —— 词库包只做
    「合并且改内容」;预设的删除墓碑保留(预设确实有删除入口)。
  - **已受影响的安装自动恢复**:读存储时清掉词库包墓碑并把干净的那份写回(幂等),老宿主设置
...
- **标签页标题进设置页**:DSH「设置 → 状态文案 → 行为」页新增「标签页标题」一组 ——
  开关、标题模板(每行一条,轮换)、空闲时的标题、轮换间隔。保存后与其余设置一起写进插件的
  配置存储(`$DSH_HOME/status-rotator/config.json`,升级不丢),不用再手改 `config.json`;
  关着的时候插件**完全不碰** `document.title`。默认仍是关闭。

### 修复

- **不再抢别人的标签页标题(与 [oh-my-dsh](https://github.com/gulagala001/oh-my-dsh) 的品牌名替换互撞)。**
  反馈:「卡死的根因是第三方插件 dsh-status-rotator 抢标题 + OMD 改品牌名互撞。」查下来冲突在我们这边,
  而且是两条路径:
  - **根因**:插件的还原分支写的是「读回来的值 ≠ 启动那一刻缓存的值就写回去」。它把
    `document.title` 当成自己的地盘,于是**任何**别的标题写者都会被顶掉 —— 包括宿主自己写的
    会话标题(`<会话名> — DeepSeek Harness`)。真浏览器实测(`omd=off` 对照):宿主写完会话标题,
    插件下一拍就把它换回启动时的产品名,标签页上再也看不到会话名。
  - **与 OMD 永不收敛**:OMD 给 `document.title` 装了一对访问器,把写入值结尾的 `DeepSeek Harness`
    换成 `Oh My DSH`(v0.1.7-rc.2.2 起,它自己 issue #12 的修法)。我们读回来的值因此**永远**不等于
    写进去的值 → 每一拍都重写一次。装载顺序还会决定表现:OMD 后装(插件先缓存了原始产品标题,
    报告人的顺序)是每拍重写;OMD 先装则是会话标题被顶成品牌名。更早那版 OMD 用观察器改标题时,
    这种高频重写会变成两边互相触发 —— 也就是「疯狂闪现 / 卡死」那类现象。
  - **修法**:标题加**所有权**概念 —— 只有「插件自己写上去的标题」才谈得上交还
    (`titleWritePlan`,纯函数);没持有过就一个字都不写,持有过则交还**最近一次别人写的标题**
    (用「当前值 ≠ 我们上次写进去的值」判断那是别人写的,而不是还一个启动快照)。卸载路径同样只交还自己的标题。
  - **判断:应当兼容,而且由我们这边修** —— 这是我们自己的缺陷(不装 OMD 也会顶掉宿主的会话标题),
    OMD 的替换是合理的产品行为,而且 OMD 的插件目录里还把 dsh-status-rotator 列为推荐插件。

### 测试

- 冒烟 333 → **342 通过 / 0 失败**:新增「标签页标题所有权」一节(`titleWritePlan` 的四种决策 +
  `normalizeConfig` 对 `title` 完整字段 / 简写 `true|false` / 非法字段逐项剔除)。
- 新增真浏览器回归 `scripts/title-coexistence-test.html`(+ `--page=title`,5 档):把
  **oh-my-dsh 的品牌替换代码逐字搬进来**,跑真实 `lib/client.js`,按 `omd=off|before|after` ×
  `title=0|1` × `flip=1` 组合量「写了几次 / DOM 变了几次 / 标题变了几次」:
  - 修前:omd=after 一档 2.6 秒窗口里 **10 次重写**、标签页停在 `Oh My DSH`(会话标题丢失);
    omd=off 也把会话标题换成了产品名 → 4 档失败。
  - 修后:5 档全过 —— 关着标题功能时写入 1 次(只有宿主那一次),开着时接管一次后稳定,
    中途关掉则**只交还一次**、标题回到宿主那一份。
- `label-layout-test.html` 的设置页渲染断言扩到「行为」页:标签页标题分组 / 开关 / 模板 / 空闲标题 /
  间隔五项都在设置页里(改设置页 JSX 时最先坏在这里)。
