...
> 把 DSH Web 底部那行 `Deep diving...` / `深度求索中...` 换成你自己的文案库:**1092 条梗、12 个主题词库包、打字机 + 白天/黑夜炫彩渐变 + 弹幕**。
...
> **dsh 0.1.7 起的状态行**:宿主把运行时文案改成了「读屏公告 span(`role="status"`,1px 视觉隐藏,内容仍是 `Deep diving...`)+ 按钮 `button[data-turn-process]` 里真正可见的标签(`Deep diving for 12s` / `深度求索中，用时12秒`,时长并进同一段文本)」。插件接管的是**按钮本身**:React 的标签元素被藏起来(它继续被每秒整段重写),插件文案与宿主时长文本摆在旁边(阶段判定与 `{elapsed}` 照旧);那个读屏公告不会被改写,屏幕阅读器听到的仍是宿主原文。0.1.6 及更早的 `role="status"` 状态行继续原样支持。
...
- **零侵入定位** — 老宿主按 `role="status"` + `aria-live="polite"`、0.1.7+ 按 `button[data-turn-process]` 定位(只藏它自己的标签、只往按钮上挂插件 span),不误伤聊天记录代码片段、其它 aria-live 区域,也不碰时钟。
...
默认词库当前共 **1092 条**,拆为 **12 个主题词库包**(核心 `phrases` 表为空——词条全部住在包里)。其中 10 个默认启用,两个 **star 包随包发布但默认关闭**——想用就在「设置 → 状态文案 → 词库包」里打开:
