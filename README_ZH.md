...
默认词库当前共 **1059 条**,拆为 **12 个主题词库包**(核心 `phrases` 表为空——词条全部住在包里)。其中 10 个默认启用,两个 **star 包随包发布但默认关闭**——想用就在「设置 → 状态文案 → 词库包」里打开:
...
| `star-route` 星标者路由 | 75 | 75 | 150 | **关** |
| **合计** | **554** | **505** | **1059** | 开 886 / 关 173 |
...
├── config.example.json     # 完整模板(默认配置 + 全部 1059 条文案,分 12 个词库包,入库)
