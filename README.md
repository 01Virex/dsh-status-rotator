...
> Replaces the DSH Web status line (`Deep diving...`) with your own phrase bank: **1102 phrases, 13 theme packs, typewriter + day/night rainbow gradient + danmaku**.
...
The default bank ships **1102 phrases**, split into **13 theme packs** (the core `phrases` table is empty — everything lives in packs). Ten packs are enabled by default; the two **star packs are shipped but off by default** — turn them on from Settings → Status Texts → Phrase packs:
...
| **total** | **583** | **519** | **1102** | 895 on / 207 off |
...
├── config.example.json     # complete template (default config + all 1102 phrases in 13 packs, committed)
