...
> Replaces the DSH Web status line (`Deep diving...`) with your own phrase bank: **1111 phrases, 13 theme packs, typewriter + day/night rainbow gradient + danmaku**.
...
The default bank ships **1111 phrases**, split into **13 theme packs** (the core `phrases` table is empty — everything lives in packs). Ten packs are enabled by default; the two **star packs are shipped but off by default** — turn them on from Settings → Status Texts → Phrase packs:
...
| **total** | **592** | **519** | **1111** | 898 on / 213 off |
...
├── config.example.json     # complete template (default config + all 1111 phrases in 13 packs, committed)
